 ...
 *
 */
class FlowIndex extends \WebApper\IndexView {
	
    /**
     * Define shortcode properties
     *
     */
	protected $shortcode = 'flow_builder';
	protected $defaults = array(
		...
		'include' => 'flow_name,flow_id,flow_type,flow_conditional,flow_statement_form_show,flow_statement_dynamic_value,flow_statement_action,flow_code,flow_hook',
	);
	...
	protected function get_records() {
		...
		global $wpdb;		
		echo json_encode(
			\SSP::simple( $_POST, $wpdb->prefix . 'web_apper_flows', 'ID', $columns ) // $_GET, $sql_details, $table, $primaryKey, $columns
		);
	}
	...
	public function get_form() {
		...
		if ( isset($_POST['web_apper_item_ids']) ) : // If rows are being edited
			if ( 1 < count($_POST['web_apper_item_ids']) ) : // If 1 rows is being edited
				...
				$modal_heading = 'Edit Flows';
				$bulk_edit = true;
			else : // Else multiple rows are being edited
				$itemID = $_POST['web_apper_item_ids'][0];
				$item = web_apper_get_flow( $itemID ); // Get the Record were calculating open seats for
				$modal_heading = 'Edit Flow';
				$bulk_edit = false;
			endif;
	...
	public function add_record() {
		...
		// Save the Flow
		$result = web_apper_insert_flow( $itemData );
		// Send ajax response
		if ( $result ) :
			return json_encode( $this->send_response( 'Flow saved.', 'Hurray!', 'alert-success' ) );  // Send Response
		else :
			return json_encode( $this->send_response( 'There was a problem saving the Flow. Please Try again.', 'Oh snap!', 'alert-error', false ) );  // Send Response
		endif;
	}
	...
	public function update_record() {
		...
		foreach( explode( ',', $_POST['web_apper_item_ids'] ) as $itemID ) : // Fields ids are a comma-delimited string here since they are coming from a PFBC form 
			$result = web_apper_update_flow( $itemID, $itemData );
			if ( !$result ) : 
				$errors[] = $itemID;
			endif;
		endforeach;
		// Send ajax response
		if ( !empty($errors) ) :
			return json_encode( $this->send_response( 'The following Flows were not updated: ' . implode(', ', $errors), 'Oh snap!', 'alert-error', false ) );  // Send Response
		else :
	...
	public function delete_record() {
		...
		foreach( $_POST['web_apper_item_ids'] as $itemID ) :
			$result = web_apper_delete_flow( $itemID );
			if ( !$result ) : 
				$errors[] = $itemID;
			endif;
		endforeach;
		// Send ajax response
		if ( !empty($errors) ) :
			return json_encode( $this->send_response( 'The following Flows were not deleted: ' . implode(', ', $errors), 'Oh snap!', 'alert-error', false ) );  // Send Response
		else :
